...
void main() {
  ...
  // 保存数据
  float arr2[UNROLLED_LOOP_INDEX];
  #pragma unroll_loop_start
  for(int i = 0; i < UNROLLED_LOOP_INDEX; i++ ){
    arr2[i] = -1.0;
  }
  ...
  // 计算点与所有边的垂线
  #pragma unroll_loop_start
  for(int i = 0; i < UNROLLED_LOOP_INDEX; i += 2){
  
    float vd = arr2[i];
    float cDD = arr2[i + 1];
    
    if(vd + 1.0 != 0.0){
      gradientAlpha2 += 1.0 / pow(vd, 2.0);
      gradientAlpha += (1.0 - 0.8 * 1.0 / pow(vd, 2.0));
    }
  }
  gradientAlpha /= gradientAlpha2;
  #pragma unroll_loop_end
  
  gl_FragColor = vec4(a_color, gradientAlpha);
}
 `;
